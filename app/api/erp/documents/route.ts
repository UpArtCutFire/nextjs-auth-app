...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[ERP DOCS] Información de sesión:', {
    ...
    if (!user) {
      console.log('[ERP DOCS] Usuario no encontrado con email:', session.user?.email);
      console.log('[ERP DOCS] Intentando buscar por ID:', (session.user as any)?.id);
      ...
      if (!user) {
        console.log('[ERP DOCS] Usuario no encontrado ni por email ni por ID');
        return NextResponse.json({ error: 'Usuario no encontrado' }, { status: 404 });
      }
      
      console.log('[ERP DOCS] Usuario encontrado por ID:', user.correo);
    }
    ...
    if (user.perfil === 'vendedor' && user.codigo_vendedor) {
      ...
      console.log('[ERP DOCS] Aplicando filtro de vendedor automático:', user.codigo_vendedor);
    }
    ...
    if (erpData && Array.isArray(erpData.data)) {
      ...
      if ((user.perfil === 'vendedor' || user.perfil === 'planificador') && user.codigo_vendedor) {
        ...
        console.log('[ERP DOCS] Documentos antes del filtro:', beforeFilter);
        console.log('[ERP DOCS] Código vendedor del usuario:', user.codigo_vendedor);
        
        if (documents.length > 0) {
          console.log('[ERP DOCS] Ejemplos de códigos vendedor en documentos:');
          documents.slice(0, 5).forEach((doc, i) => {
            console.log(`[ERP DOCS] Doc ${i + 1}: CodVend="${doc.CodVend}", Vendedor="${doc.Vendedor}"`);
          });
        }
      ...
      if ((user.perfil === 'vendedor' || user.perfil === 'planificador') && user.codigo_vendedor) {
        const beforeFilter = documents.length;
        console.log('[ERP DOCS] Documentos antes del filtro (objeto):', beforeFilter);
        console.log('[ERP DOCS] Código vendedor del usuario:', user.codigo_vendedor);
        
        if (documents.length > 0) {
          console.log('[ERP DOCS] Ejemplos de códigos vendedor en documentos (objeto):');
          documents.slice(0, 5).forEach((doc, i) => {
            console.log(`[ERP DOCS] Doc ${i + 1}: CodVend="${doc.CodVend}", Vendedor="${doc.Vendedor}"`);
          });
        }
        ...
        console.log('[ERP DOCS] Documentos filtrados por vendedor (objeto):', beforeFilter, '->', documents.length);
      }
    } else {
