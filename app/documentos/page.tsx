...
export default function DocumentosPage() {
  ...
  // Estados para mostrar verificaciones existentes (administradores)
  const [viewVerificationsModalOpen, setViewVerificationsModalOpen] = useState(false);
  const [existingVerifications, setExistingVerifications] = useState<any[]>([]);
  const [loadingVerifications, setLoadingVerifications] = useState(false);

  // Estados para despacho
  const [dispatchModalOpen, setDispatchModalOpen] = useState(false);
  const [selectedDocumentForDispatch, setSelectedDocumentForDispatch] = useState<ERPDocument | null>(null);
  ...
  // Efecto para establecer el código de vendedor automáticamente para vendedores y planificadores
  useEffect(() => {
    if (session?.user) {
      const user = session.user as any;
      // Si es vendedor o planificador, establecer su código automáticamente
  ...
  const isEligibleForPaymentVerification = (doc: ERPDocument): boolean => {
    ...
    // Debug logs para planificador
    if (user?.perfil === 'planificador') {
      console.log('🔍 DEBUG Planificador - Verificando elegibilidad:', {
        perfil: user.perfil,
        docTipo: doc.TipoDoc,
    ...
    // Vendedores y planificadores pueden verificar pagos
    // Administradores pueden ver verificaciones existentes
    if (!user) return false;
    
    const canVerify = user.perfil === 'vendedor' || user.perfil === 'planificador' || user.perfil === 'administrador';
    if (!canVerify) {
      console.log('❌ Usuario no puede verificar:', user.perfil);
      return false;
    }
    ...
    const isValidState = doc.EstadoProcesoDoc === 'A';
    
    if (user?.perfil === 'planificador') {
      console.log('📊 Resultado elegibilidad:', {
        isValidType,
        isValidState,
        resultado: isValidType && isValidState
      });
    }
  ...
  const openPaymentVerificationModal = async (doc: ERPDocument) => {
    const user = session?.user as any;
    
    // Si es administrador, buscar y mostrar verificaciones existentes
    if (user?.perfil === 'administrador') {
      setLoadingVerifications(true);
      setSelectedDocumentForVerification(doc);
      
      try {
        // Buscar verificaciones existentes para este documento
        const response = await fetch(`/api/payment-verifications?documentNumber=${doc.NumDoc}&documentType=${doc.TipoDoc}`);
        if (response.ok) {
          const data = await response.json();
          setExistingVerifications(data.verifications || []);
          setViewVerificationsModalOpen(true);
        } else {
          toast.error('Error al obtener verificaciones');
        }
      } catch (error) {
    ...
    // Para vendedores y planificadores, abrir modal de creación
    setSelectedDocumentForVerification(doc);
    setPaymentVerificationModalOpen(true);
  };

  // Función para abrir modal de despacho
  const openDispatchModal = (doc: ERPDocument) => {
    setSelectedDocumentForDispatch(doc);
      ...
      <div className="space-y-6">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if ((session?.user as any)?.perfil === 'administrador') {
                        setFilters(prev => ({ ...prev, CodVend: e.target.value }));
                      }
                    }}
                    placeholder={((session?.user as any)?.perfil === 'vendedor' || (session?.user as any)?.perfil === 'planificador') ? 'Código asignado automáticamente' : 'Ej: VEN001'}
              ...
              <div className="mt-4">
                <Button 
                  ...
                  onClick={() => {
                    ...
                    // Mantener el código de vendedor para vendedores y planificadores
                    const vendorCode = (user?.perfil === 'vendedor' || user?.perfil === 'planificador') && user?.codigo_vendedor 
        ...
        <DispatchModal
          ...
          onClose={() => {
            ...
            setSelectedDocumentForDispatch(null);
          }}
          documentNumber={selectedDocumentForDispatch?.NumDoc || ''}
          documentType={selectedDocumentForDispatch?.TipoDoc || ''}
          vendorCode={selectedDocumentForDispatch?.CodVend || ''}
          clienteNombre={selectedDocumentForDispatch?.NomCliente || ''}
          documentInfo={selectedDocumentForDispatch}
          document={selectedDocumentForDispatch}
        />
        ...
        {/* Modal para ver verificaciones existentes (Administradores) */}
        <Dialog open={viewVerificationsModalOpen} onOpenChange={setViewVerificationsModalOpen}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <ShieldCheck className="h-5 w-5" />
                Verificaciones de Pago del Documento
              </DialogTitle>
              <DialogDescription>
                {selectedDocumentForVerification ? (
                  <>
                    Documento {selectedDocumentForVerification.TipoDoc} #{selectedDocumentForVerification.NumDoc} - {selectedDocumentForVerification.NomCliente}
                  </>
                ) : (
                  'Información de verificaciones del documento'
                )}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4">
              {loadingVerifications ? (
                <div className="flex items-center justify-center py-8">
                  <Loader2 className="h-8 w-8 animate-spin" />
                  <span className="ml-2">Cargando verificaciones...</span>
                </div>
              ) : existingVerifications.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  <ShieldCheck className="h-12 w-12 mx-auto mb-4 opacity-50" />
                  <p>No se encontraron verificaciones para este documento</p>
                </div>
              ) : (
                <div className="grid gap-4">
                  {existingVerifications.map((verification, index) => (
                    <Card key={verification.id || index} className="p-4">
                      <div className="space-y-3">
                        <div className="flex items-center justify-between pb-2 border-b">
                          <div className="flex items-center gap-2">
                            <ShieldCheck className="h-4 w-4 text-muted-foreground" />
                            <span className="font-medium text-sm">Verificación {index + 1}</span>
                          </div>
                          <Badge variant={verification.status === 'APPROVED' ? 'default' : verification.status === 'REJECTED' ? 'destructive' : 'secondary'}>
                            {verification.status === 'APPROVED' ? 'Aprobada' : verification.status === 'REJECTED' ? 'Rechazada' : 'Pendiente'}
                          </Badge>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className="text-sm font-medium">{verification.user?.nombre || 'Desconocido'}</p>
                          </div>
                          
                          <div>
                            <Label className="text-xs text-muted-foreground">Código Vendedor</Label>
                            <p className="text-sm font-medium">{verification.vendorCode || '-'}</p>
                          </div>
                          
                          <div>
                            <Label className="text-xs text-muted-foreground">Método de Pago</Label>
                            <p className="text-sm font-medium capitalize">{verification.paymentMethod || '-'}</p>
                          </div>
                          
                          <div>
                            <Label className="text-xs text-muted-foreground">Monto Verificado</Label>
                            <p className="text-sm font-medium">
                              ${(verification.amount || 0).toLocaleString('es-CL')}
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <p className="text-sm font-medium">
                              {new Date(verification.createdAt).toLocaleDateString('es-CL')} {new Date(verification.createdAt).toLocaleTimeString('es-CL')}
                            </p>
                          </div>
                          
                          {verification.approvedAt && (
                            <div>
                              <Label className="text-xs text-muted-foreground">Aprobado Por</Label>
                              <p className="text-sm font-medium">{verification.approver?.nombre || '-'}</p>
                            </div>
                          )}
                        </div>
                        ...
                        {verification.rejectionReason && (
                          <div>
                            <Label className="text-xs text-muted-foreground">Razón de Rechazo</Label>
